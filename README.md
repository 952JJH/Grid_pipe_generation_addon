...
3. Unzip the repository. You should find the required `Grid_pipe_generation_addon.zip` and `gcode-exporter-master.zip` inside
4. In Blender, navigate to `edit` > `Preferences` > `Add-ons` > `Install` from File  and find and select the `Grid_pipe_generation_addon.zip` and `gcode-exporter-master.zip` file in this repository and activate it.
...
5. Adjust the settings in the sidebar to change the results in real time.
...
6. Select all the generated structures, right-click, and choose Join.
...
7. Adjust the position of the joined structure, then export the G-code using the settings provided.
